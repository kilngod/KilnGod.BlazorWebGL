...
{
    // PGA3 from bivector.net
    public enum Basis { s, e0, e1, e2, e3, e01, e02, e03, e12, e31, e23, e021, e013, e032, e123, e0123 };

    public class GeometricAlgebra
    {
        // just for debug and print output, the basis names
        Basis[] _basis = { Basis.s, Basis.e0, Basis.e1, Basis.e2, Basis.e3, Basis.e01, Basis.e02, Basis.e03, Basis.e12, Basis.e31, Basis.e23, Basis.e021, Basis.e013, Basis.e032, Basis.e123, Basis.e0123 };
        ...
        public GeometricAlgebra(float f = 0f, int idx = 0)
        {
            _mVec[idx] = f;
        }

        public GeometricAlgebra(string textBasis)
        {
            textBasis = textBasis.Replace(" ", "").Replace("*","").Replace("+", " +").Replace("-"," -");
            string[] basisValues = textBasis.Split(" ".ToCharArray());
            foreach (string basisValue in basisValues)
            {
                for(Basis i = Basis.e0123; i>=0; i--)
                {
                    if (basisValue.IndexOf(i.ToString())> 0)
                    {
                        _mVec[Convert.ToInt32(i)] = Convert.ToSingle(basisValue.Substring(0, basisValue.IndexOf(i.ToString())-1));
                        break;
                    }
                    if (i == 0)
                    {
                        _mVec[0] = Convert.ToSingle(basisValue[Convert.ToInt32(i)]);
                    }
                }
            }
        }
        ...
        /// </summary>
        public static GeometricAlgebra operator ~(GeometricAlgebra a)
        {
            GeometricAlgebra res = new GeometricAlgebra();
            res[0] = a[0];
            res[1] = a[1];
            res[2] = a[2];
            res[3] = a[3];
        ...
        /// </summary>
        public GeometricAlgebra Conjugate()
        {
            GeometricAlgebra res = new GeometricAlgebra();
            res[0] = this[0];
            res[1] = -this[1];
        ...
        /// Main involution
        /// </summary>
        public GeometricAlgebra Involute()
        {
            GeometricAlgebra res = new GeometricAlgebra();
            res[0] = this[0];
            res[1] = -this[1];
        ...
        /// </summary>
        public static GeometricAlgebra operator *(GeometricAlgebra a, GeometricAlgebra b)
        {
            GeometricAlgebra res = new GeometricAlgebra();
        ...
        /// </summary>
        public static GeometricAlgebra operator *(float a, GeometricAlgebra b)
        {
            GeometricAlgebra res = new GeometricAlgebra();
            res[0] = a * b[0];
            res[1] = a * b[1];
        ...
        /// </summary>
        public static GeometricAlgebra operator *(GeometricAlgebra a, float b)
        {
            GeometricAlgebra res = new GeometricAlgebra();
            res[0] = a[0] * b;
            res[1] = a[1] * b;
        ...
        /// </summary>
        public float norm() { return MathF.Sqrt(MathF.Abs((this * Conjugate())[0])); }
        ...
        /// </summary>
        public GeometricAlgebra normalized() { return this * (1 / norm()); }


        // PGA is plane based. Vectors are planes. (think linear functionals)
        public static GeometricAlgebra e0 = new GeometricAlgebra(1f, 1);
        public static GeometricAlgebra e1 = new GeometricAlgebra(1f, 2);
        public static GeometricAlgebra e2 = new GeometricAlgebra(1f, 3);
        public static GeometricAlgebra e3 = new GeometricAlgebra(1f, 4);

        // PGA lines are bivectors.
        public static GeometricAlgebra e01 = e0 ^ e1;
        public static GeometricAlgebra e02 = e0 ^ e2;
        public static GeometricAlgebra e03 = e0 ^ e3;
        public static GeometricAlgebra e12 = e1 ^ e2;
        public static GeometricAlgebra e31 = e3 ^ e1;
        public static GeometricAlgebra e23 = e2 ^ e3;

        // PGA points are trivectors.
        public static GeometricAlgebra e123 = e1 ^ e2 ^ e3; // the origin
        public static GeometricAlgebra e032 = e0 ^ e3 ^ e2;
        public static GeometricAlgebra e013 = e0 ^ e1 ^ e3;
        public static GeometricAlgebra e021 = e0 ^ e2 ^ e1;

        /// <summary>
        /// PGA3D.plane(a,b,c,d)
        /// A plane is defined using its homogenous equation ax + by + cz + d = 0
        /// </summary>
        public static GeometricAlgebra plane(float a, float b, float c, float d) { return a * e1 + b * e2 + c * e3 + d * e0; }

        /// <summary>
        /// PGA3D.point(x,y,z)
        /// A point is just a homogeneous point, euclidean coordinates plus the origin
        /// </summary>
        public static GeometricAlgebra point(float x, float y, float z) { return e123 + x * e032 + y * e013 + z * e021; }

        /// <summary>
        /// Rotors (euclidean lines) and translators (ideal lines)
        /// </summary>
        public static GeometricAlgebra rotor(float angle, GeometricAlgebra line) { return MathF.Cos(angle / 2.0f) + MathF.Sin(angle / 2.0f) * line.normalized(); }
        public static GeometricAlgebra translator(float dist, GeometricAlgebra line) { return 1.0f + dist / 2.0f * line; }
        ...
        public static GeometricAlgebra circle(float t, float radius, GeometricAlgebra line)
        {
            return rotor(t * 2.0f * MathF.PI, line) * translator(radius, e1 * e0);
        }

        // a torus is now the product of two circles. 
        public static GeometricAlgebra torus(float s, float t, float r1, GeometricAlgebra l1, float r2, GeometricAlgebra l2)
        {
            return circle(s, r2, l2) * circle(t, r1, l1);
        }

        // and to sample its points we simply sandwich the origin ..
        public static GeometricAlgebra point_on_torus(float s, float t)
        {
        ...
        public override string ToString()
        {
            ...
            for (int i = 0; i < 16; ++i)
                if (_mVec[i] != 0.0f)
                {
                    sb.Append($"{_mVec[i]}{(i == 0 ? string.Empty : _basis[i].ToString())} + ");
                    n++;
                }
            if (n == 0) sb.Append("0");
